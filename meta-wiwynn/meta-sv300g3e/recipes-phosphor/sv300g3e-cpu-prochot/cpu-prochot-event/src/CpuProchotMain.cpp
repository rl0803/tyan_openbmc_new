...
{
    int ok = gpiod_ctxless_get_value("0", gpioNumPwrGdSysRwrOk,
                                    false, "cpu-prochot-event");
    if (1 != ok)
    {
        return false;
    }
    ...
    acpiPowerMatch = std::make_unique<sdbusplus::bus::match::match>(
        ...
        [](sdbusplus::message::message& message) {
            ...
            if (findState != values.end())
            {
                ...
                // To avoid the prochot glitch issue at early power-on stage.
...
bool createCpuProchotObjects(
    boost::asio::io_service& io,
    std::shared_ptr<sdbusplus::asio::connection>& conn,
    std::vector<std::unique_ptr<CpuProchotObject>>& prochotObjs)
{
    std::ifstream gpioConfigFile(gpioConfigPath);

    if(!gpioConfigFile)
    {
        std::cerr << "Failed to open CPU Prochot GPIO config file: "
                  << gpioConfigPath <<"\n";
...
/*
            sd_journal_print(LOG_ERR, "[Idx %d][GPIO %d][Name %s][Type %s]\n",
                            idx, gpioNumber, sensorName.c_str(), triggerType.c_str());
*/
            gpiod_line* line = nullptr;
            struct gpiod_line_request_config config
            {
                "cpu-prochot-event", GPIOD_LINE_REQUEST_EVENT_BOTH_EDGES, 0
            };
            ...
            {
/*
                sd_journal_print(LOG_ERR, "[MAP Type: %s|%d]\n",
                                (findEvent->first).c_str(), findEvent->second);
*/
                config.request_type = findEvent->second;
            }

            prochotObjs.push_back(std::make_unique<CpuProchotObject>(
                                 io, conn, line, config, sensorName));
        }
...
int main(int argc, char *argv[])
{
    ...
    systemBus->request_name("xyz.openbmc_project.CpuProchotEvent");
    ...
    if(!createCpuProchotObjects(io, systemBus, prochotObjs))
    {
        std::cerr << "Failed to create CPU Prochot objects\n";
        return -1;
    }
